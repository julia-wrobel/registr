...
author: "Julia Wrobel and Erin McDonnell"
date: "`r Sys.Date()`"
...
registr_parametric = register_fpca(Y = registration_data, family = "binomial", 
                                   Kt = 8, Kh = 3, npc = 1, gradient = FALSE,
...
Each `prior` argument takes a vector of length 2, which includes the mean and standard deviation for the given knot coordinate's normally-distributed prior. In the code above, the priors suggest that the two knots should lie near $(0.2, 0.3)$ and $(0.8, 0.6)$, with equal variances of 1 across all four priors. As expected, smaller variances lead to stronger tendency toward the prior means, as demonstrated in the warping function plots below.
...
In some cases it may be of interest to place periodic boundary conditions on the B-spline basis functions for the population-level and subject-specific mean templates. The periodic conditions ensure that the resulting function starts and ends with the same value. This may be useful when modeling cyclical data, such as daily physical activity patterns. To use periodic B-spline basis functions during registration, use the following code:
