...
\end{eqnarray*}
$$
For subject $i$, inverse warping functions $h_i^{-1}$ map unregistered time $t_i^*$ to registered time $t$ such that $h_i^{-1}(t_i^*) = t$. $Y_i\left(t_i^*\right)$ and $Y_i\left(h_i^{-1}(t_i^*)\right)$ are the unregistered and registered response curves, respectively. The subject-specific means $\mu_i(t)$ are related to the population-level mean $\alpha(t)$ and a linear combination of population-level basis functions $\psi(t)$ and subject-specific scores $c_i$ through a known link function $g$. 
...
The results of registering the continuous data are plotted below.

```{r plot_reg_gauss, echo = FALSE, fig.show='hold', fig.width=2}
Y = registr_gauss$Y

ggplot(Y, aes(tstar, latent_mean, group = id)) + theme_bw() + 
	geom_line(alpha = 0.25)

ggplot(Y, aes(t, latent_mean, group = id)) + theme_bw() + 
	geom_line(alpha = 0.25) 

ggplot(Y, aes(t_hat, latent_mean, group = id)) + theme_bw() + 
	geom_line(alpha = 0.25) 
